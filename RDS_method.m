function [range_axis,doppler_axis,CUT]=RDS_method(startframe,endframe,mot)
    ...
    % radar parameter
    fs =77e9; % start frequency
    Tsweep = 0.0625./5; % Sweep time in ms
...
%     hold on
    ...
    for va=(1:vitualarray)
        for n=(1:NTS)
            ...
            RDMabs(n,:,va)=abs(fftshift(fft(data(n,:,va),numchirp,2),2));
        end
...
%     colormap; %xlim([1 9])
%     clim = get(gca,'CLim');
%     set(gca, 'CLim', clim(2)+[-40,0]);  % max-40 to max
